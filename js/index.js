window.addEventListener("load", () => {
  ...
  EditorUi.prototype.init = function () {
    ...
    this.actions.addAction("new...", function () {
      ...
      if (answer) {
        EditorUi.prototype.editor.setGraphXml(
          window.parent.mxUtils.parseXml(
            '<mxGraphModel dx="667" dy="662" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="826" pageHeight="1169" background="#ffffff"><root><mxCell id="0"/><mxCell id="1" parent="0"/></root></mxGraphModel>'
          ).documentElement
        );
      }
    });
    ...
    EditorUi.prototype.editor.graphChangeListener = function (
      sender,
      eventObject
    ) {
    ...
    if (room && username) {
      ...
      if (room && !username) {
        this.footerContainer.innerHTML =
          "<strong>AVISO: " +
          UserNotFound +
          " no encontrado. MODO SOLITARIO</strong>";
        ...
        this.footerContainer.innerHTML =
          ...
          " y " +
          RoomNotFound +
          " no encontrados. MODO SOLITARIO</strong>";
      }
    }
  };
  ...
  EditorUi.prototype.sidebar.prototype.itemClicked = function (
    cells,
    ds,
    evt,
    elt
  ...
  socket.on("connect", () => {
    socket.on("draw_component", (data) => {
      ...
      if (xmlString !== "") {
        EditorUi.prototype.editor.setGraphXml(
          window.parent.mxUtils.parseXml(xmlString).documentElement
        );
      }
    });
    ...
    socket.on("load_room_title", (data) => {
      ...
      title.innerHTML =
        '<center style="margin-right:10%;"><strong>' +
        data.title.trim().toUpperCase() +
        "</strong> </center>";
      EditorUi.prototype.sidebarContainer.insertBefore(
        title,
        EditorUi.prototype.sidebarContainer.firstChild
      );
    });
    ...
    socket.on("error_server", () => {
      EditorUi.prototype.footerContainer.innerHTML =
        "<strong>AVISO: Error al traer datos del Servidor. MODO SOLITARIO</strong>";
    });
