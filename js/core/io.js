...
const addUser = (id, name, room) => {
  const existingUser = users.find(
    (user) => user.id.trim().toLowerCase() === id.trim().toLowerCase()
  );
...
io.on("connection", (socket) => {
  ...
  socket.on("login", async ({ name, room }) => {
    ...
    const responseUser = await axios.get(
      "http://3.135.145.84:80/api/user/" + name
    );
    ...
    const responseProject = await axios.get(
      "http://3.135.145.84:80/api/cargar-diagrama/" + room
    );

    if (
      responseUser.status === 200 &&
      responseProject.status === 200 &&
      responseUser.data.name &&
      responseProject.data.nombre
    ) {
      ...
      if (!error) {
        ...
        let xmlString = "";
        if (!findRoom(room)) {
          console.log('findRoom', responseProject.data.content)
        ...
        socket.emit("load_room_title", {
          title: responseProject.data.nombre,
          users: usersInRoom,
        });
  ...
  socket.on("save_component", async (data) => {
    if (findRoom(data.room)) {
      ...
      const response = await axios.put(
        "http://3.135.145.84:80/api/guardar-diagrama/" + data.room,
        body
      );
    }
  });
